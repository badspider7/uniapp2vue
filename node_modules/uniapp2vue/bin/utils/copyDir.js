...
const copyProjectName = `${projectName}_backup`; //拷贝文件夹的名字
...
fs.copySync(basicProjectPath, path.join(newDirPath, copyProjectName, 'src'), { filter: ignoreDir })


//把之前漏掉的文件移动到新文件夹中
ignoreArr.forEach((item) => {
    try {
        let newFilename = item.slice(item.lastIndexOf('\\')+1);
        fs.copySync(item, path.join(newDirPath, copyProjectName,newFilename))
    } catch(err) {
        console.log('复制src之外的文件失败',err)
    }
})
